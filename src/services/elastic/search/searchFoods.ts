...
const removeDuplicates = (foods: FoodResultWithScore[]): FoodResultWithScore[] => {
  const fields = <const>["description", "category", "brand", "data_source"];
  const uniqueFoods = _.uniqWith(foods, (a, b) => fields.every((field) => a[field] === b[field]));
  return uniqueFoods;
};

const prioritizeMoreNutrients = (foods: FoodResultWithScore[]): FoodResultWithScore[] => {
...
const prioritizeStartOfDescription = (foods: FoodResultWithScore[], searchTerm: string): FoodResultWithScore[] => {
  ...
  const searchWords = PorterStemmer.tokenizeAndStem(searchTerm.toLocaleLowerCase());
  const prioritizedFoods = foods.map((food) => {
    const startOfDescription = PorterStemmer.tokenizeAndStem(food.description?.toLocaleLowerCase() || "").slice(
      0,
      Math.max(searchWords.length, 2)
    );
...
const prioritizeExactMatches = (foods: FoodResultWithScore[], searchTerm: string) => {
  const prioritizedFoods = foods.map((food) => {
...
const searchFoods = async (searchTerm: string): Promise<FoodResultWithScore[]> => {
  const resp = (await ElasticClient.search("food", searchTerm, elasticOptions)) as SearchResponse;
  ...
  const foodsWithScores = foodsWithPortions.map((food) => ({
    ...food,
    searchScore: resp.results.find((result) => parseInt(result.id.raw, 10) === food.id)?._meta.score || 0,
  }));
